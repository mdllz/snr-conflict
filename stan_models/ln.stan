...
parameters{
  ...
  real theta_mu;                    // population-level effect
  real<lower=0> alpha_sigma; // individual differences intercept
  real<lower=0> theta_sigma; // individual differences conflict effect
  vector[I] alpha;                       
  vector[I] theta_raw;
...
model{
  ...
  for(n in 1:N){
    y[n] ~ lognormal(alpha[subj[n]] + b[n]*theta[subj[n]], sigma[subj[n]]);
  }
}
...
generated quantities{
  ...
  for(n in 1:N){
    log_lik[n] = lognormal_lpdf(y[n] | alpha[subj[n]] + (b[n]*theta[subj[n]]), sigma[subj[n]]);
    y_pred[n] =  lognormal_rng(alpha[subj[n]] + b[n]*theta[subj[n]], sigma[subj[n]]);
  }

...
```{r 'snr-computation-functions'}
# functions
# variance of single drift rate in case of two drift rates
V_drift <- function(L, beta, v){
  E_v <- sqrt(L)*(gamma(L-1.5)/gamma(L-1))*((1/(L*beta))+v)
  E_v2 <- (L/(L-2))*(v^2 + (3/(L^2 * beta^2)) + ((3*v)/(L*beta)))
  res <- E_v2 - E_v^2
  return(res)
}

# covariance of drift rate
Cov_drift <- function(L, beta, v1, v2){
  E_v1v2 <- (L/(L-2)) * ((1/(L*beta)) + v1) * ((1/(L*beta)) + v2)
  E_v1 <- sqrt(L)*(gamma(L-1.5)/gamma(L-1))*((1/(L*beta))+v1)
  E_v2 <- sqrt(L)*(gamma(L-1.5)/gamma(L-1))*((1/(L*beta))+v2)
  res <- E_v1v2 - (E_v1*E_v2)
  return(res)
}

# variance of drift rate difference v1-v2
V_effect <- function(L, v1, v2, beta){
  V_v1 <- V_drift(L, beta, v1)
  V_v2 <- V_drift(L, beta, v2)
  Cov_v1v2 <- Cov_drift(L, beta, v1, v2)
  res <- V_v1 + V_v2 - (2* Cov_v1v2)
  return(res)
}

snr_ratio <- function(rdm_extract, trialnr){
  
  I <- ncol(rdm_extract$drift_1_c) # nr of participants
  niter <- nrow(rdm_extract$drift_1_c)
  S <- niter# Nr of samples from posterior distributions
  idx <- seq(1, niter, by = niter/S)
  
  li <- trialnr[trialnr$cond=="inc",]$n # individual correct trial numbers per condition
  lc <- trialnr[trialnr$cond=="c",]$n
  
  ratio2 <- numeric(S)
  sigma2_mean <- numeric(S)
  ...
  for(s in 1:S){
    ...
    sigma2 <-  rep(NA, I)
    for(i in 1:I){
      effect_var[i] <- V_effect(L = mean(li[i], lc[i]), v1 = rdm_extract$drift_1_c[idx_thin,i],
                                v2 = rdm_extract$drift_1_inc[idx_thin,i], beta = rdm_extract$beta[idx_thin,i])  
      sigma2[i] <- (effect_var[i]*mean(li[i], lc[i]))/2
    }
    sigma2_mean[s] <- mean(sigma2)
    sigma_theta[s] <- rdm_extract$theta_sigma[idx_thin]
    ratio2[s] <- (sigma_theta[s]^2/sigma2_mean[s]) # average effect variance and trial number across individuals
    print(s)
  }
  
  res <- list(ratio_a = sqrt(ratio2), sigma_theta = sigma_theta, sigma = sqrt(sigma2_mean))
...
pp_check_overall_rdm <- function(data, modelfit, npred){
  ...
  pp_rdm <- ggplot2::ggplot(plot_dat_individual_pp2, aes(x = rt, y = p_corr)) +
...
individual_pp_check <- function(data, modelfit, npred = 500){
  ...
  qq_rdm_c <- left_join(pp_deciles_obs, pp_deciles) %>% 
  ...
  qq_rdm_ic <- left_join(pp_deciles_obs, pp_deciles) %>% 
    filter(congruency == -0.5) %>% 
    ggplot(aes(rt_obs, rt, label = s)) +
    geom_point() +
    geom_abline(intercept =0 , slope = 1) +
  ...
  return(list(congruent = qq_rdm_c,
              incongruent = qq_rdm_ic))
}

```


```{r 'von-Bastian-RDM-fitting', eval = F}
...
I_vonbastian <- length(unique(vonbastian$subject))
 ...
 dat_vonbastian <- vonbastian %>% 
   ...
   mutate(accuracy = ifelse(accuracy == 1, 1, 2),
          congruency = ifelse(congruency == 1, 0.5, -0.5)) %>% # opposite coding to LNR
   as.matrix()

 
start_values <- function(){list(psi_mu = runif(1, 0.1, 0.2),
                                psi_sigma = runif(1, 0.01, 0.05),
                                psi = rep(runif(1, 0.1, 0.2), I_vonbastian),
                                alpha_1 = rep(runif(1, 2,4), I_vonbastian),
                                alpha_1_mu = runif(1, 2,4),
                                alpha_1_sigma = runif(1, 0.01, 0.05),
                                alpha_2 = runif(1, 1,2),
                                theta = rep(runif(1, 0.1,1), I_vonbastian),
                                theta_mu = runif(1,0.1,0.7),
                                theta_sigma = runif(1, 0.01, 0.05),
                                beta = rep(runif(1, 1,2),I_vonbastian),
                                beta_mu = runif(1, 1,2),
                                beta_sigma = runif(1,0,0.1))} 
...
data_vB <- list(N = nrow(vonbastian), I = I_vonbastian, K = 2, Y = dat_vonbastian, 
                ...
                subj = as.numeric(as.factor(vonbastian$subject))) 
 
stan_rdm_vB <- rstan::stan("stan_models/rdm.stan", data = data_vB, init = start_values, 
                               iter = 3000, warmup = 2000, chains = 4, 
                               control = list(adapt_delta = 0.98, stepsize = 0.4))

rdm_extract_vB <- rstan::extract(stan_rdm_vB, pars = c("drift_1_c", "drift_1_inc", "beta", "psi", "theta_sigma"))
...
#save(rdm_extract_vB, trialnr_vB, file="output/RDM/rdm_samplesfor_ratio_vB.RData")


#saveRDS(stan_rdm_vB, "output/RDM/rdm_vonbastian.RDS")

# posterior predictive checks --------------------------------------------------

plot_overall_rdm_vB <- pp_check_overall_rdm(vonbastian, stan_rdm_vB, 500)
ggsave("output/RDM/pp_rdm_vonBastian.pdf", plot_overall_rdm_vB, width = 4.71, height = 2.52, dev = cairo_pdf)

pdf(file = "output/RDM/plot_cdf_pp_vonbastian.pdf", width = 7, height = 3.5)
par(mfrow=c(1,2))    
plot_cdf_pp(data = vonbastian, modelfit = stan_rdm_vB, subject = "none", factors = "CT", 
          layout = NULL, xlim = c(0,5))  
dev.off()


individual_pp_vB <- individual_pp_check(vonbastian, stan_rdm_vB)
ggsave("output/RDM/ind_rdm_c_vB.pdf",individual_pp_vB$congruent, height = 5.8, width = 5.2, dev = cairo_pdf)
ggsave("output/RDM/ind_rdm_ic_vB.pdf",individual_pp_vB$incongruent, height = 5.8, width = 5.2, dev = cairo_pdf)
```

```{r 'Rey-Mermet-RDM-fitting', eval = F}
...
start_values <- function(){list(psi_mu = runif(1, 0.1, 0.2),
                                psi_sigma = runif(1, 0.01, 0.05),
                                psi = rep(runif(1, 0.1, 0.2), I_reymermet),
                                alpha_1 = rep(runif(1, 2,4), I_reymermet),
                                alpha_1_mu = runif(1, 2,4),
                                alpha_1_sigma = runif(1, 0.01, 0.05),
                                alpha_2 = runif(1, 1,2),
                                theta = rep(runif(1, 0.1,1), I_reymermet),
                                theta_mu = runif(1,0.1,0.7),
                                theta_sigma = runif(1, 0.01, 0.05),
                                beta = rep(runif(1, 1,2),I_reymermet),
                                beta_mu = runif(1, 1,2),
                                beta_sigma = runif(1,0,0.1))} 
...
stan_rdm_RMy <- rstan::stan("stan_models/rdm.stan", data = data_RMy, init = start_values, 
                               iter = 3000, warmup = 2000, chains = 4, 
                               control = list(adapt_delta = 0.98, stepsize = 0.4))

rdm_extract_RMy <- rstan::extract(stan_rdm_RMy, pars = c("drift_1_c", "drift_1_inc", "beta", "psi", "theta_sigma"))
...
#save(rdm_extract_RMy, trialnr_RMy, file="output/RDM/rdm_samplesfor_ratio_RMy.RData")


#saveRDS(stan_rdm_RMy, "output/RDM/rdm_reymermet.RDS")

# posterior predictive checks --------------------------------------------------


plot_overall_rdm_RMy <- pp_check_overall_rdm(reymermet, stan_rdm_RMy, 500)
ggsave("output/RDM/pp_rdm_ReyMermet.pdf", plot_overall_rdm_RMy, width = 4.71, height = 2.52, dev = cairo_pdf)

pdf(file = "output/RDM/plot_cdf_pp_ReyMermet.pdf", width = 7, height = 3.5)
par(mfrow=c(1,2))    
plot_cdf_pp(data = reymermet, modelfit = stan_rdm_RMy, subject = "none", factors = "CT", 
          layout = NULL, xlim = c(0,5))  
dev.off()


individual_pp_RMy <- individual_pp_check(reymermet, stan_rdm_RMy)
ggsave("output/RDM/ind_rdm_c_RMy.pdf",individual_pp_RMy$congruent, height = 5.8, width = 5.2, dev = cairo_pdf)
ggsave("output/RDM/ind_rdm_ic_RMy.pdf",individual_pp_RMy$incongruent, height = 5.8, width = 5.2, dev = cairo_pdf)
```

```{r 'Pratte-RDM-fitting', eval = F}
...
start_values <- function(){list(psi_mu = runif(1, 0.1, 0.2),
                                psi_sigma = runif(1, 0.01, 0.05),
                                psi = rep(runif(1, 0.1, 0.2), I_pratte),
                                alpha_1 = rep(runif(1, 2,4), I_pratte),
                                alpha_1_mu = runif(1, 2,4),
                                alpha_1_sigma = runif(1, 0.01, 0.05),
                                alpha_2 = runif(1, 1,2),
                                theta = rep(runif(1, 0.1,1), I_pratte),
                                theta_mu = runif(1,0.1,0.7),
                                theta_sigma = runif(1, 0.01, 0.05),
                                beta = rep(runif(1, 1,2),I_pratte),
                                beta_mu = runif(1, 1,2),
                                beta_sigma = runif(1,0,0.1))} 
...
stan_rdm_P <- rstan::stan("stan_models/rdm.stan", data = data_P, init = start_values, 
                               iter = 3000, warmup = 2000, chains = 4, 
                               control = list(adapt_delta = 0.98, stepsize = 0.4))

rdm_extract_P <- rstan::extract(stan_rdm_P, pars = c("drift_1_c", "drift_1_inc", "beta", "psi", "theta_sigma"))
...
#save(rdm_extract_P, trialnr_P, file="output/RDM/rdm_samplesfor_ratio_P.RData")

#saveRDS(stan_rdm_P, "output/RDM/rdm_pratte.RDS")

# posterior predictive checks --------------------------------------------------


plot_overall_rdm_P <- pp_check_overall_rdm(pratte, stan_rdm_P, 500)
ggsave("output/RDM/pp_rdm_pratte.pdf", plot_overall_rdm_P, width = 4.71, height = 2.52, dev = cairo_pdf)

pdf(file = "output/RDM/plot_cdf_pp_pratte.pdf", width = 7, height = 3.5)
par(mfrow=c(1,2))    
plot_cdf_pp(data = pratte, modelfit = stan_rdm_P, subject = "none", factors = "CT", 
          layout = NULL, xlim = c(0,5))  
dev.off()


individual_pp_P <- individual_pp_check(pratte, stan_rdm_P)
ggsave("output/RDM/ind_rdm_c_P.pdf",individual_pp_P$congruent, height = 5.8, width = 5.2, dev = cairo_pdf)
ggsave("output/RDM/ind_rdm_ic_P.pdf",individual_pp_P$incongruent, height = 5.8, width = 5.2, dev = cairo_pdf)
```

```{r 'Enkavi-RDM-fitting', eval = F}
...
start_values <- function(){list(psi_mu = runif(1, 0.1, 0.2),
                                psi_sigma = runif(1, 0.01, 0.05),
                                psi = rep(runif(1, 0.1, 0.2), I_enkavi),
                                alpha_1 = rep(runif(1, 2,4), I_enkavi),
                                alpha_1_mu = runif(1, 2,4),
                                alpha_1_sigma = runif(1, 0.01, 0.05),
                                alpha_2 = runif(1, 1,2),
                                theta = rep(runif(1, 0.1,1), I_enkavi),
                                theta_mu = runif(1,0.1,0.7),
                                theta_sigma = runif(1, 0.01, 0.05),
                                beta = rep(runif(1, 1,2),I_enkavi),
                                beta_mu = runif(1, 1,2),
                                beta_sigma = runif(1,0,0.1))} 
...
stan_rdm_E <- rstan::stan("stan_models/rdm.stan", data = data_E, init = start_values, 
                               iter = 3000, warmup = 2000, chains = 4, 
                               control = list(adapt_delta = 0.98, stepsize = 0.4))

rdm_extract_E <- rstan::extract(stan_rdm_E, pars = c("drift_1_c", "drift_1_inc", "beta", "psi", "theta_sigma"))
...
#save(rdm_extract_E, trialnr_E, file="output/RDM/rdm_samplesfor_ratio_E.RData")


#saveRDS(stan_rdm_E, "output/RDM/rdm_enkavi.RDS")

# posterior predictive checks --------------------------------------------------


plot_overall_rdm_E <- pp_check_overall_rdm(enkavi, stan_rdm_E, 500)
ggsave("output/RDM/pp_rdm_enkavi.pdf", plot_overall_rdm_E, width = 4.71, height = 2.52, dev = cairo_pdf)

pdf(file = "output/RDM/plot_cdf_pp_enkavi.pdf", width = 7, height = 3.5)
par(mfrow=c(1,2))    
plot_cdf_pp(data = enkavi, modelfit = stan_rdm_E, subject = "none", factors = "CT", 
          layout = NULL, xlim = c(0,5))  
dev.off()


individual_pp_E <- individual_pp_check(enkavi, stan_rdm_E)
ggsave("output/RDM/ind_rdm_c_E.pdf",individual_pp_E$congruent, height = 5.8, width = 5.2, dev = cairo_pdf)
ggsave("output/RDM/ind_rdm_ic_E.pdf",individual_pp_E$incongruent, height = 5.8, width = 5.2, dev = cairo_pdf)
```
...
## Pratte et al. -----------------------------
load("output/RDM/rdm_samplesfor_ratio_P.RData")

res_P <- snr_ratio(rdm_extract_P, trialnr_P)

#saveRDS(res_P, "output/RDM/ratio_rdm_P_a.RDS")


## Rey-Mermet et al. -----------------------------------
load("output/RDM/rdm_samplesfor_ratio_RMy.RData")

res_RMy <- snr_ratio(rdm_extract_RMy, trialnr_RMy)

saveRDS(res_RMy, "output/RDM/ratio_rdm_RMy_a.RDS")

## Von Bastian et al. -------------------------------------
load("output/RDM/rdm_samplesfor_ratio_vB.RData")

res_vB <- snr_ratio(rdm_extract_vB, trialnr_vB)

saveRDS(res_vB, "output/RDM/ratio_rdm_vB_a.RDS")

## Enkavi et al. ---------------------------------------
load("output/RDM/rdm_samplesfor_ratio_E.RData")

res_E <- snr_ratio(rdm_extract_E, trialnr_E)

saveRDS(res_E, "output/RDM/ratio_rdm_E_a.RDS")

```

```{r 'SN-ratios-RDM-sim', eval = F}

# functions --------------------------------------------------------------------
rWald <- function(n,B,v,A=0)
...
var_dv <- function(dtc,dti) {
  # mean
  mc <- apply(dtc,2,mean)
  mi <- apply(dti,2,mean)
  # q
  qc <- sapply(1:ncol(dtc), function(i) vQ(L = nrow(dtc), mn = mc[i], dtc[, i]))
  qi <- sapply(1:ncol(dti), function(i) vQ(L = nrow(dti), mn = mi[i], dti[, i]))
  q <- 0.5*(qc + qi)
  var(vWald(mc,q)-vWald(mi,q))
}  

getv <- function(s,i,samps,li,lc,r) {
  var_dv(dtc=matrix(rWald(lc[i]*r,samps$beta[s,i],samps$drift_1_c[s,i]),nrow=lc[i],ncol=r),
         dti=matrix(rWald(li[i]*r,samps$beta[s,i],samps$drift_1_inc[s,i]),nrow=li[i],ncol=r))
}


# Pratte et al. ----------------------------------------------------------------

load("rdm_samplesfor_ratio_P.RData")
I <- ncol(rdm_extract_P$drift_1_c) # nr of participants
S <- 500# Nr of samples from posterior distributions
niter <- nrow(rdm_extract_P$drift_1_c)
idx <- seq(1, niter, by = niter/S)
r=10000 # nr of replicates 

li <- trialnr_P[trialnr_P$cond=="inc",]$n # individual correct trial numbers per condition
...
for(s in 1:S){
  idx_thin <- idx[s]
  effect_var <- unlist(mclapply(1:I,getv,s=idx_thin, li=li, lc=lc, r=r,samps=rdm_extract_P,mc.cores=16))
  print(effect_var)
  sigma_theta[s] <- rdm_extract_P$theta_sigma[idx_thin]
...
saveRDS(ratio, "output/ratio_rdm_P.RDS")
save(effect_var_mean, sigma_theta, ratio, file = "output/ratio_and_components_rdm_P.RData")

#rm(list = ls())

# Von Bastian et al.  ---------------------------------------------------
load("rdm_samplesfor_ratio_vB.RData")
I <- ncol(rdm_extract_vB$drift_1_c) # nr of participants
S <- 500# Nr of samples from posterior distributions
niter <- nrow(rdm_extract_vB$drift_1_c)
idx <- seq(1, niter, by = niter/S)
r=10000 # nr of replicates 

li <- trialnr_vB[trialnr_vB$cond=="inc",]$n # individual correct trial numbers per condition
...
for(s in 1:S){
  idx_thin <- idx[s]
  effect_var <- unlist(mclapply(1:I,getv,s=idx_thin, li=li, lc=lc, r=r,samps=rdm_extract_vB,mc.cores=16))
  sigma_theta[s] <- rdm_extract_vB$theta_sigma[idx_thin]
  effect_var_mean[s] <- mean(effect_var)
  ratio[s] <- (sigma_theta[s]^2/((effect_var_mean[s]*mean_L)/2)) # average effect variance and trial number across individuals
  print(s)
}
...
saveRDS(ratio, "output/ratio_rdm_vB.RDS")
save(effect_var_mean, sigma_theta, ratio, file = "output/ratio_and_components_rdm_vB.RData")

#rm(list = ls())

# Rey-Mermet et al.  ---------------------------------------------------
load("rdm_samplesfor_ratio_RMy.RData")
I <- ncol(rdm_extract_RMy$drift_1_c) # nr of participants
S <- 500# Nr of samples from posterior distributions
niter <- nrow(rdm_extract_RMy$drift_1_c)
idx <- seq(1, niter, by = niter/S)
r=10000 # nr of replicates 

li <- trialnr_RMy[trialnr_RMy$cond=="inc",]$n # individual correct trial numbers per condition
...
for(s in 1:S){
  idx_thin <- idx[s]
  effect_var <- unlist(mclapply(1:I,getv,s=idx_thin, li=li, lc=lc, r=r,samps=rdm_extract_RMy,mc.cores=16))
  sigma_theta[s] <- rdm_extract_RMy$theta_sigma[idx_thin]
  effect_var_mean[s] <- mean(effect_var)
  ratio[s] <- (sigma_theta[s]^2/((effect_var_mean[s]*mean_L)/2)) # average effect variance and trial number across individuals
  print(s)
}
...
saveRDS(ratio, "output/ratio_rdm_RMy.RDS")
save(effect_var_mean, sigma_theta, ratio, file = "output/ratio_and_components_rdm_RMy.RData")

#rm(list = ls())

# Enkavi et al.-----------------------------------------------------------------

load("rdm_samplesfor_ratio_E.RData")
I <- ncol(rdm_extract_E$drift_1_c) # nr of participants
S <- 500# Nr of samples from posterior distributions
niter <- nrow(rdm_extract_E$drift_1_c)
idx <- seq(1, niter, by = niter/S)
r=10000 # nr of replicates 

li <- trialnr_E[trialnr_E$cond=="inc",]$n # individual correct trial numbers per condition
...
for(s in 1:S){
  idx_thin <- idx[s]
  effect_var <- unlist(mclapply(1:I,getv,s=idx_thin, li=li, lc=lc, r=r,samps=rdm_extract_E,mc.cores=16))
  sigma_theta[s] <- rdm_extract_E$theta_sigma[idx_thin]
  effect_var_mean[s] <- mean(effect_var)
  ratio[s] <- (sigma_theta[s]^2/((effect_var_mean[s]*mean_L)/2)) # average effect variance and trial number across individuals
  print(s)
}
...
saveRDS(ratio, "output/ratio_rdm_E.RDS")
save(effect_var_mean, sigma_theta, ratio, file = "output/ratio_and_components_rdm_E.RData")

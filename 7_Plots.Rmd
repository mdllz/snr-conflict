...
sn_plot_by_data <- sn_ratios %>% 
  ...
  scale_color_viridis_d(option = "plasma", end = 0.8, limits = rev, labels = c("Normal", "Lognormal", "Shifted-lognormal", "Lognormal race", "Racing diffusion")) +
  scale_y_discrete(limits = rev) +
  ggtitle("") +
  labs(x = "Signal-to-noise ratio" ~ gamma, y = "", color = "") +
  geom_vline(xintercept = 0, col = "grey70") +
  #scale_x_continuous(limits = c(0, 0.38)) +
  mytheme(base_size = 12) 

sn_plot_by_models <- sn_ratios %>%
  mutate(idx = as.character(case_when(Model == "Lognormal" ~ 4,
                         Model == "Shifted-lognormal" ~ 3,
                         Model == "LNR" ~ 2,
                         Model == "RDM" ~ 1,
                         Model == "Normal" ~ 5))) %>% 
  ggplot(aes(y = as.character(idx), x = signa, xmin = signa.lower, 
             xmax = signa.upper, color = dataset)) +
  geom_pointinterval(position = position_dodge(width = .5)) +
  scale_color_viridis_d(option = "C", end = 0.8) +
  scale_y_discrete(labels = c("Racing diffusion", "Lognormal race", "Shifted-lognormal","Lognormal", "Normal")) +
  ggtitle("") +
  labs(x = "Signal-to-noise ratio", y = "", color = "") +
  geom_vline(xintercept = 0, col = "grey70") +
  #scale_x_continuous(limits = c(0, 0.38)) +
  mytheme(base_size = 12)
...
plot_samples <- function(model, title, samples_vB, samples_P, samples_RMy, samples_E,y_labels = FALSE){
  ...
  samples <- samples_vB %>% 
    ...
    bind_rows(samples_E %>% 
                ...
                mutate(Dataset = "Enkavi et al., 2019")) %>% 
    group_by(Dataset) %>% 
    summarize(samples = median({{model}}),
...
p_signal_n <- plot_samples(n, "Normal", signal_vB, signal_P, signal_RMy, signal_E,y_labels = TRUE) + theme(axis.text.x = element_text(size = 8))
p_signal_ln <- plot_samples(ln, "Lognormal", signal_vB, signal_P, signal_RMy, signal_E)+ theme(axis.text.x = element_text(size = 8))
p_signal_sln <- plot_samples(sln, "Shifted Lognormal", signal_vB, signal_P, signal_RMy, signal_E) + labs(x = expression("Signal " ~ sigma[theta]))+ theme(axis.text.x = element_text(size = 8))
p_signal_lnr <- plot_samples(lnr, "Lognormal race", signal_vB, signal_P, signal_RMy, signal_E)+ theme(axis.text.x = element_text(size = 8))
p_signal_rdm <- plot_samples(rdm, "Racing diffusion", signal_vB, signal_P, signal_RMy, signal_E)+ theme(axis.text.x = element_text(size = 8))
...
p_noise_n <- plot_samples(n, "Normal", noise_vB, noise_P, noise_RMy, noise_E,y_labels = TRUE)+ theme(axis.text.x = element_text(size = 8)) + ggtitle("")
p_noise_ln <- plot_samples(ln, "Lognormal", noise_vB, noise_P, noise_RMy, noise_E)+ theme(axis.text.x = element_text(size = 8)) + ggtitle("")
p_noise_sln <- plot_samples(sln, "Shifted Lognormal", noise_vB, noise_P, noise_RMy, noise_E) + labs(x = expression("Noise " ~ sigma))+ theme(axis.text.x = element_text(size = 8)) + ggtitle("")
p_noise_lnr <- plot_samples(lnr, "Lognormal Race", noise_vB, noise_P, noise_RMy, noise_E)+ theme(axis.text.x = element_text(size = 8)) + ggtitle("")
p_noise_rdm <- plot_samples(rdm, "Racing Diffusion", noise_vB, noise_P, noise_RMy, noise_E)+ theme(axis.text.x = element_text(size = 7)) + ggtitle("")
...
png('/Users/udo/Dropbox/to read/DASC model/NumericalExamplesvhatRDM2.png', res = 72*4, height = 480*4, width = 480*4)
...
png('/Users/udo/Dropbox/to read/DASC model/NumericalExamplesCovRDM.png', res = 72*4, height = 480*4, width = 480*4)
